...
### 결정 시점이 온 항목은 먼저 물어본다

[GDD 12장](docs/GDD.md) 미결정 표에 **결정 시점** 열이 있다. 주차나 후속 단계가 그 시점에
닿으면, 사용자가 묻지 않아도 **내가 먼저 꺼낸다.**

작업 직전에 물어야 판단이 산다. 지나쳐 버리면 이미 짜둔 구조를 나중에 다시 손대게 되고,
너무 일찍 꺼내면 근거가 될 진행 상황이 아직 없다.

- 주차가 바뀔 때와 1차 완성 후 후속 단계로 넘어갈 때 표를 훑는다
- 정해지면 **결정 완료** 표로 옮기고 근거를 남긴다. 미결정 표에서는 지운다
