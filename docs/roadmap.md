...
```
회복량        healAmount 10 — 적 30마리에 HP 30 이면 부족하다 (v0.4 빌드 결과)
spawnCount    Room_2 가 difficulty 1 인데 8마리, Room_4 가 12마리 — 난이도와 따로 논다
강화 계수     DamagePerLevel 0.15 · SpeedPerLevel 0.07
              교체 계승이 -1 없이 그대로라 총 강화량이 순증한다 — 조일 곳은 여기다
```

**`#127` 로 보상방이 선택이 됐으니 이제 회복량만 올리는 대응은 안 맞는다.** *"무기를 바꿔
처리 속도를 올릴까, 회복할까"* 가 난이도의 답이 되도록 잡는다.
