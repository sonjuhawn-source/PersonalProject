...
[#115](../../issues/115) 보스 다종화(랜덤 선택 + 패턴 확장)를 넣고 싶었지만 **나흘 안에는
안 들어간다.** 보스 하나에 프리팹·애니메이터·패턴 SO 셋이 붙는데 `#105` 에서 그게 하루였다.

**패턴 확장만 떼어내는 것도 안 권한다** — `AttackPattern` SO 를 더 만드는 건 싸지만
(`interruptible` · `maxHeightDiff` 로 이미 조절된다) **밸런스를 다시 봐야 하고**, 그 시간이
`#96` 적 구분보다 뒤에 있다. 지금 보스는 패턴 셋으로 충분히 읽힌다.
