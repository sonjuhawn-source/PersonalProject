...
> 작성 중 — 캡처에 번호를 매기는 작업이 남았다.
>
> 화면별로 영역을 나누고 무엇을 하는 자리인지 적는다. 캡처는
> [../../Screenshots/](../../Screenshots/) 에 있다.

---

## 쓰는 법

1. `Screenshots/` 의 이미지를 열어 **아래 표의 번호대로 영역에 사각형과 숫자를 그린다**
2. 번호를 매긴 이미지를 `Screenshots/numbered/` 에 같은 이름으로 저장한다
3. 표는 그대로 쓴다 — 번호와 설명이 이미 맞춰져 있다

번호를 매기는 도구는 윈도우 기본 그림판으로 충분하다. 사각형 색은 캡처와 대비되는
한 가지로 통일한다 (예시 자료는 청록색을 썼다).
...
| 2 | 무기 슬롯 | 두 칸과 각 무기의 강화 수치. 활성 무기가 위 |
